...
import requests
import random
import unicodedata  # Added to handle fancy characters
...
@bot.event
async def on_ready():
    print(f'Logged in as {bot.user} - Anti-Bypass Watchdog Active!')
...
async def on_message(message):
    ...
    # --- 🛠️ 1. NORMALIZE TEXT (Handles ƀíţçħ, ky5, etc.) ---
    # First, strip accents and weird symbols (turns í into i, etc.)
    normalized = unicodedata.normalize('NFKD', content).encode('ascii', 'ignore').decode('utf-8')
    
    # Second, replace numbers with letters
    clean_content = normalized.replace('5', 's').replace('1', 'i').replace('0', 'o').replace('3', 'e').replace('4', 'a').replace('7', 't').replace('$', 's').replace('@', 'a')
    ...
    # --- 🛡️ 5. SMART WATCHDOG WITH ADVANCED BYPASS LOGGING ---
    FLAGGED_WORDS = [
