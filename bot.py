    ...
    if not is_bad:
        ...
        if repo_url:
            ...
            try:
                ...
                if response.status_code == 200:
                    files = response.json()
                    # Updated exclusion logic to prevent specific images from appearing in random rotation
                    images = [f for f in files if f['name'].lower().endswith(('.png', '.jpg', '.jpeg', '.gif')) and f['name'] not in [REBUTTAL_FILE, TARGET_ASTRO_IMAGE]]
                    if images:
