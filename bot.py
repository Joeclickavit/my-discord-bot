...
# --- 🛰️ NEW: GLOBAL SYSTEM STATE ---
astro_system_active = False
...
@bot.event
async def on_ready():
    print(f'Logged in as {bot.user} - Watchdog with System Toggle Active!')

@bot.event
async def on_message(message):
    global astro_system_active
    ...
    # --- 🕹️ SYSTEM TOGGLE LOGIC ---
    if "system on" in clean_content:
        astro_system_active = True
        await message.reply("🛰️ **System Initialized.** Monitoring for specific keywords...")
        return

    if "shut off" in clean_content:
        astro_system_active = False
        await message.reply("💤 **System Deactivated.** Standing down.")
        return

    # --- 🌈 THE "GAY" TRIGGER (Only works when System is ON) ---
    if astro_system_active and "gay" in words_in_message:
        await message.reply(f"<@{TARGET_USER_ID}>, Please Admit it bro i'm the gay moon and i legit know your gay")
