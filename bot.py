...
async def on_message(message):
    ...
    # --- 🌈 2. GAY TRIGGER (Ping + GitHub Image ONLY) ---
    if "gay" in words_in_message:
        # Fixed: Ensuring the URL uses the raw usercontent subdomain for proper embedding
        encoded_path = quote(GAY_FLOWER_FILE)
        image_url = f"https://raw.githubusercontent.com/{GAY_FLOWER_REPO}/main/{encoded_path}"
        # Sending the ping and the direct link so Discord generates the embed
