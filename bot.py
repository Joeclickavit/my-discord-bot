...
async def on_message(message):
    ...
    clean_content = content.replace('5', 's').replace('1', 'i').replace('0', 'o').replace('3', 'e').replace('4', 'a').replace('7', 't').replace('$', 's').replace('@', 'a')
    ...
    FLAGGED_WORDS = [
        ...
        "rape", "rapist", "raped", "rappel", "gangbang",

        # --- Trump/Supporter Phrases (Multi-word phrases) ---
        "make america great again", "maga", "trump 2024", "trump 2026", "trump 2028", 
        "trump train", "charlie kirk", "ultra maga", "keep america great", "kag",
        "america first", "drain the swamp", "fake news", "witch hunt", "stop the steal",
        "build the wall", "lock her up", "truth social", "don junior", "vance 2028",
        "golden age of america", "americas revival", "save america", "47th president",
        "drill baby drill", "rino", "crooked joe", "laffin kamala", "project 2025",
        "border czar", "mass deportations", "deport them all"
    ]
    ...
    for flagged in FLAGGED_WORDS:
        # Check if it's a multi-word phrase or just a word
        if " " in flagged:
    ...
    if is_bad:
        ...
        # Log message with Username and Original Content
        log_report = (
    ...
    # Only runs if the message wasn't flagged for bad words
    if not is_bad:
