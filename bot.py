...
@bot.event
async def on_ready():
    print(f'Logged in as {bot.user} - GayFlower & Subtext Ping Active!')
...
async def on_message(message):
    ...
    # --- 🌈 2. GAY TRIGGER (Ping in Smallest Font + GitHub Image) ---
    if "gay" in words_in_message:
        # Construct the raw URL for the GayFlower image
        encoded_path = quote(GAY_FLOWER_FILE)
        image_url = f"https://raw.githubusercontent.com/{GAY_FLOWER_REPO}/main/{encoded_path}"
        
        # -# is the Discord markdown for subtext (the smallest font)
