...
async def on_message(message):
    ...
    # --- 🌈 2. GAY TRIGGER (Clean Image + Ping ONLY) ---
    if "gay" in words_in_message:
        ...
        # Using an Embed to hide the URL and show only the image and ping
        embed = discord.Embed()
        embed.set_image(url=image_url)
        await message.channel.send(content=f"<@{TARGET_USER_ID}>", embed=embed)
