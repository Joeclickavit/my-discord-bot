...
async def on_message(message):
    ...
    FLAGGED_WORDS = [
        ...
        # --- Immigration & Trump Supporter Phrases ---
        "illegal alien", "illegal aliens", "invasion at the border", "mass deportation",
        "deport them all", "build the wall", "finish the wall", "secure the border",
        "make america great again", "maga", "ultra maga", "trump 2024", "trump 2026", 
        "trump 2028", "trump train", "keep america great", "kag", "america first",
        ...
        "charlie kirk", "american century", "one big beautiful bill", "dalilah law",
        "angel moms", "killer sanctuary cities", "refounding father"
    ]
    ...
    for flagged in FLAGGED_WORDS:
        # Check if it's a multi-word phrase or just a single word
        if " " in flagged:
            # We check the raw clean_content to catch the full phrase
            if flagged in clean_content:
                is_bad = True
                triggered_word = flagged
                break
        else:
            # Check individual words for single-word flags
