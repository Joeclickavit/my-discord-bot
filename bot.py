...
async def on_message(message):
    ...
    # --- 🌈 2. GAY TRIGGER (Image First, then Reply Ping) ---
    if "gay" in words_in_message:
        encoded_path = quote(GAY_FLOWER_FILE)
        image_url = f"https://raw.githubusercontent.com/{GAY_FLOWER_REPO}/main/{encoded_path}"
        # Send image with brackets to hide the raw URL text
        img_msg = await message.channel.send(f"<{image_url}>")
        # Reply to the image message with the ping
        await img_msg.reply(f"<@{TARGET_USER_ID}>")
    ...
    if is_bad:
        ...
        log_report = (
            ...
            # Using mention instead of string for author to ensure it logs right
